...
export default class CategoriesModal extends PureComponent {
  ...
  handleStartScroll = (e) => {
    this.setState({
      isScrolling: true,
      scrollOffsetY: e.nativeEvent.contentOffset.y,
    })
  }
  ...
  handlePanResponderMove = (e, gesture) => {
    ...
    if (this.isDropZone(gesture)) {
      ...
      return Animated.spring(this.removeBtnScale, {
        toValue: 2,
  ...
  handleSetDragMode = () => {
    this.setState({ isDragMode: true }, () => {
      Animated.timing(this.removeBtnOpacity, {
        toValue: 1,
        duration: 300,
        useNativeDriver: true,
      }).start()
  ...
  handleOffDragMode = (transTypeId) => {
    ...
    Animated.spring(this.removeBtnScale, {
      toValue: 1,
      useNativeDriver: true,
    }).start()
    Animated.timing(this.removeBtnOpacity, {
  ...
  handleSetDropZonePosition = ({ nativeEvent }) => {
    UIManager.measure(nativeEvent.target,
  ...
  handleSetScrollContentSize = (width, height) => this.setState({
    scrollSize: {
      width,
      height,
    },
