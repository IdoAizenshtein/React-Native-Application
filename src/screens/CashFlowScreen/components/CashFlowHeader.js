...
export default class CashFlowHeader extends PureComponent {
  ...
  UNSAFE_componentWillReceiveProps (props) {
    if (this.scrollViewTop && this.scrollViewTop._component &&
      ((props.categoriesMatchCopy &&
        (props.categoriesMatchCopy !== this.props.categoriesMatchCopy)) ||
  ...
  render () {
    ...
    let textDates = AppTimezone.moment(dateFromTimestamp).format('DD/MM/YY') +
      ' - ' + AppTimezone.moment(dateTillTimestamp).format('DD/MM/YY')
    if (AppTimezone.moment(dateFromTimestamp).endOf('month').valueOf() ===
      dateTillTimestamp) {
      textDates = t('cashFlow:tillEndOfThisMonth')
    } else if (AppTimezone.moment(dateFromTimestamp)
      .add(2, 'months')
      .date(0)
      .valueOf() === dateTillTimestamp) {
                        ...
                        >
                          <Text style={{
                            ...
                            this.props.searchkey.find(
                              ...
                              ? this.props.searchkey.find(
                                (it) => it.paymentDescription === f.id).name
                              : '')}
                          </Text>
                          <View style={{
                            marginTop: 2,
