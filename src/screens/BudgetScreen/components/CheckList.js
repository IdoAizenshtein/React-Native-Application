...
class MyListItem extends PureComponent {
  ...
  _onPress = () => {
    if (this.props.type === 'transFrequencyNameSolek' &&
      (this.props.id === 'WEEK' || this.props.id === 'MONTH')) {
  ...
  render () {
    ...
    return (
      <View>
        <View style={[
          ...
          <View style={{
            ...
            <TouchableOpacity
              ...
              {(selected && ((this.props.id === 'USER_DEFINED_TOTAL' &&
                ((this.props.targetTypeDIRECTD) || this.props.type ===
                  'autoUpdateTypeName_CCARD_TAZRIM' || this.props.type ===
                  'autoUpdateTypeNameDIRECTD')) ||
                (this.props.type === 'autoUpdateTypeName_SOLEK_TAZRIM' &&
                  (this.props.id === 'USER_DEFINED_TOTAL')))) && (
                <View style={{
                  ...
                  <TextInput
                    ...
                    onEndEditing={(e) => {
                      this.setState({
                        totalVal: e.nativeEvent.text.toString()
                          .replace(/[^\d.]/g, ''),
                      })
                      this._onPressTotal(
                    ...
                    onChangeText={(totals) => {
                      if (String(totals).split('.').length > 2) {
                        ...
                        this.setState({
                          totalVal: totals.toString()
                            .replace(/[^\d.]/g, ''),
                        })
                        this._onPressTotal(
                          totals.toString().replace(/[^\d.]/g, ''))
                      }
                    }}
                    value={this.state.totalVal
                      ? inputWorkaround.getWorkaroundChar() +
                      String(this.state.totalVal)
                      : null}
              ...
              <Text
                ...
                ? AppTimezone.moment(this.props.expirationDate)
                  .format('DD/MM/YY')
        ...
        {(selected && ((this.props.type === 'transFrequencyNameSolek' &&
          (this.props.id === 'WEEK' || this.props.id === 'MONTH')))) && (
          <View style={[
            cs(!this.props.isRtl, commonStyles.row, [commonStyles.rowReverse]),
            {
                  ...
                  <ScrollView
                    ...
                    }}>
                    {
                      (this.props.id === 'MONTH' ? [
                        ...
                        29,
                        30,
                        31].reverse() : [1, 2, 3, 4, 5]).map((item, i) => {
                        return (<TouchableOpacity
                          style={[
                            cs(this.props.isRtl, commonStyles.row,
                              ...
                              height: 34,
                              borderRadius: 17,
                              backgroundColor: (this.props.id === 'MONTH'
                                ? (item === frequencyDay)
                                : (this.days[item - 1].toUpperCase() ===
                                  frequencyDay)) ? '#08d3b8' : 'transparent',
                            }]}
                          key={i.toString()}
                          onPress={this._onPressFrequencyDay(
                            this.props.id === 'MONTH' ? item : this.days[item -
                            1].toUpperCase())}><View>
                          <Text style={{
                            fontSize: sp(15),
                            width: 34,
                            ...
                            color: ((this.props.id === 'MONTH'
                              ? item
                              : this.days[item - 1].toUpperCase()) ===
                              frequencyDay) ? '#ffffff' : '#0f3860',
                          }}>
                            {this.props.id === 'MONTH'
                              ? item
                              : LocaleConfig.locales.he.dayNamesShort[item - 1]}
                          </Text>
    ...
    setTimeout(() => {
      ...
      if (!(this.props.type === 'transFrequencyNameSolek' &&
        ...
        !(this.props.type === 'autoUpdateTypeName_SOLEK_TAZRIM' &&
          (id === 'USER_DEFINED_TOTAL')) &&
        !((this.props.targetTypeDIRECTD || this.props.type ===
          'autoUpdateTypeNameDIRECTD' || this.props.type ===
          'autoUpdateTypeName_CCARD_TAZRIM') && id === 'USER_DEFINED_TOTAL') &&
        id !== 'times' && !(this.props.type === 'endDate' && id === 'on')) {
        this.props.close()
      }
    }, 10)
  }
  ...
  render () {
    ...
    return (
      <View>
        ...
        {(((this.props.type === 'autoUpdateTypeName_SOLEK_TAZRIM' &&
          this.props.value.autoUpdateTypeName === 'USER_DEFINED_TOTAL'))) && (
          <View style={{
            marginTop: 10,
            alignItems: 'center',
            flexDirection: 'row-reverse',
